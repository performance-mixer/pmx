...
int main(int argc, char **argv) {
  ...
  builder.set_filter_name("pmx-osc-network-router")
      .set_media_type("Osc")
      ...
      .add_parameter("target.protocol", 3, pwcpp::filter::variant_type("udp"))
      .add_signal_processor([&queue, &condition_variable](
                                auto position, auto &in_ports, auto &out_ports,
        ...
        if (buffer.has_value()) {
          ...
          if (pod.has_value()) {
            if (spa_pod_is_sequence(pod.value())) {
              auto sequence =
                  reinterpret_cast<struct spa_pod_sequence *>(pod.value());
  ...
  std::thread network_sender([&running, &queue, &lock, &condition_variable,
                              &app]() {
    auto ip_address =
        get<std::string>(app.value()->parameter_collection.parameters[0].value);
    auto port = get<int>(app.value()->parameter_collection.parameters[1].value);
    auto protocol =
        get<std::string>(app.value()->parameter_collection.parameters[2].value);
    ...
    while (running) {
      ...
      if (queue.pop(message)) {
        ...
        std::cout << "No message, waiting..." << std::endl;
