...
int main(int argc, char *argv[]) {
  ...
  builder.set_filter_name("pmx-traktor-z1-router")
      ...
      .add_signal_processor([&queue, &previous, &is_init](
          ...
          if (queue.read_available() > 0) {
            ...
            while (queue.pop(message)) {
              if (is_init) {
                ...
                std::array<
                    std::optional<std::tuple<std::string, std::uint16_t>>, 14>
                    changes{};
                ...
                if (previous.cue_mix != message.cue_mix) {
                  changes[10] = std::make_tuple("/cue_mix", message.cue_mix);
                }
                ...
                if (previous.cross_fader != message.cross_fader) {
                  changes[13] =
                      std::make_tuple("/cross_fader", message.cross_fader);
                }
                ...
                if (std::any_of(changes.begin(), changes.end(),
                  ...
                  for (auto &&change :
                       changes | std::views::filter([](const auto &change) {
                         return change.has_value();
                       })) {
