...
std::expected<void, error::error> console::setup_command(
  ...
  const auto filter_chain_ctrl_proxy = std::find_if(
    proxies.begin(), proxies.end(), [](const auto &proxy) {
      return proxy.name == "pmx-filter-chain-ctrl";
    });
  ...
  const auto filter_chain_ctrl_id = filter_chain_ctrl_proxy->object_id;
  const auto pw_filter_chain_proxy = proxy_watcher.get_proxy(
    filter_chain_ctrl_id);
  ...
  const auto layer_proxy = std::find_if(proxies.begin(), proxies.end(),
                                        [](const auto &proxy) {
                                          return proxy.name ==
  ...
  const auto group_a_proxy = std::find_if(proxies.begin(), proxies.end(),
                                          [](const auto &proxy) {
                                            return proxy.name ==
  ...
  const auto group_b_proxy = std::find_if(proxies.begin(), proxies.end(),
                                          [](const auto &proxy) {
                                            return proxy.name ==
  ...
  const auto inputs_proxy = std::find_if(proxies.begin(), proxies.end(),
                                         [](const auto &proxy) {
                                           return proxy.name ==
