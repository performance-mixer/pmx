...
local function connect_lp_mini_init_output_to_mk3_input()
...
local function connect_lp_mini_init_input_to_mk3_output()
...
SimpleEventHook({
    ...
    interests = {
        EventInterest({
            ...
            Constraint({ "port.alias", "=", "pmx-lp-mini-init:lp_mini_out" }),
        })
    },
    execute = function(event)
        ...
        connect_lp_mini_init_output_to_mk3_input()
    end
}):register()
...
SimpleEventHook({
    ...
    interests = {
        EventInterest({
            Constraint({ "event.type", "=", "port-added" }),
            Constraint({ "port.alias", "=", "pmx-lp-mini-init:lp_mini_in" }),
        })
    },
    execute = function(event)
        ...
        connect_lp_mini_init_input_to_mk3_output()
    end
}):register()
...
SimpleEventHook({
    ...
    interests = {
        EventInterest({
            Constraint({ "event.type", "=", "port-added" }),
            Constraint({ "port.alias", "=", "Launchpad Mini MK3:Launchpad Mini MK3 LPMiniMK3 MI" }),
